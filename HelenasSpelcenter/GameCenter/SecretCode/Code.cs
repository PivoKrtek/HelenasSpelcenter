...
namespace HelenasSpelcenter
{
    class Code
    {
        ...
        public static void CompareCodeWithSecretCode(int whichToCompare)
        {
            ...
            int[] copySecretCode = new int[SecretCodeGame.LengthOfCode];
            for (int i = 0; i < SecretCode.CodeList.Count; i++)
            {
                copySecretCode[i] = SecretCode.CodeList[i].Number;
            }
            int[] copyGuess = new int[SecretCodeGame.LengthOfCode];
        ...
        {
            CodeRevealed = numberOfRightGuessed == SecretCodeGame.LengthOfCode;
            if (CodeRevealed)
            { SecretCodeGame.EndTime = DateTime.Now; }
        }
